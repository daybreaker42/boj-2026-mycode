...
def dfs(i, j):
    ...
    if plain_input[i][j]:
        ...
        while stack:
            temp = stack.popleft()
            if plain_input[temp[0]][temp[1]] == '1' and [temp[0], temp[1]] not in visited:
                neighbor.append([temp[0], temp[1]])
                visited.append([temp[0], temp[1]])

                if temp[0] > 0 and plain_input[temp[0]-1][temp[1]] == '1':    # 위
                    stack.appendleft([temp[0]-1, temp[1]])
                if temp[1] > 0 and plain_input[temp[0]][temp[1]-1] == '1':    # 왼쪽
                    stack.appendleft([temp[0], temp[1]-1])
                if temp[0] < n-1 and plain_input[temp[0]+1][temp[1]] == '1':  # 아래
                    stack.appendleft([temp[0]+1, temp[1]])
                if temp[1] < n-1 and plain_input[temp[0]][temp[1]+1] == '1':  # 오른쪽
                    stack.appendleft([temp[0], temp[1]+1])
